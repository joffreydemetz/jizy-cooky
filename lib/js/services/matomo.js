...
export default class MatomoService extends Service {
    constructor() {
        ...
        // Instance-level assignments: Service base class initialises
        // `this.js = null` and `this.fallback = true`, which would shadow
        // prototype methods of the same name and skip both branches of
        // Service.exec(). Binding lambdas here keeps the dispatch working.
        this.js = () => this._loadMatomo();
    ...
    _loadMatomo() {
        if (!this.matomoLoaded) {
            ...
            let domains = Config.get('service.matomo.domains');
            if (!domains && domain) {
                domains = [domain];
            }
            ...
            const cookieDomain = Config.get('service.matomo.cookieDomain');
            if (cookieDomain) {
                _paq.push(['setCookieDomain', cookieDomain]);
            }

            const titlePrefix = Config.get('service.matomo.titlePrefix');
            if (titlePrefix) {
                _paq.push(['setDocumentTitle', titlePrefix + '/' + document.title]);
            }

            const trackPrefix = Config.get('service.matomo.trackPrefix');
            if (trackPrefix) {
                _paq.push(['setCustomUrl', location.protocol + '//' + location.hostname + '/' + trackPrefix + location.pathname]);
            }
